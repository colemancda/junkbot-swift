...
// All of the game's setup + run loop lives here as `junkbotMain()` (plus the module-level
// state every other file draws/reads through) rather than in `main.swift` top-level code, so
// the exact same sources can build two ways:
// - Desktop executables (`ports/SDL3`): `main.swift` is just a `junkbotMain()` call.
// - Android (`ports/Android`): there is no process `main` at all - SDL's Java `SDLActivity`
//   loads the game as a shared library and invokes its exported `SDL_main` on a dedicated
//   thread, and that wrapper (ports/Android/Sources/JunkbotAndroid/AndroidMain.swift) calls
//   `junkbotMain()`. Library targets can't contain top-level code, hence this split.
//
// One behavioral consequence: as ordinary globals (not `main.swift` top-level variables) all
// the `let`s below are initialized *lazily on first access*, not eagerly in source order -
// `junkbotMain()` is written to touch them in dependency order (SDL.initialize before
// `window`, `windowWidth`/`windowHeight` assigned before `window`'s initializer reads them).

// MARK: - Repo-relative paths

/// Overrides `repoRoot` entirely when set (before `junkbotMain()` runs, i.e. before anything
/// touches `repoRoot`). Android sets this to the internal-storage directory the APK's bundled
/// assets were extracted into (see ports/Android/Sources/JunkbotAndroid/AndroidMain.swift) -
/// none of the strategies below can work there (no bundle, no executable path, no source tree).
nonisolated(unsafe) public var assetRootOverridePath: String? = nil

/// Locates the directory containing `images/`, `font/`, `levels/`, `audio/`. Strategies, tried
/// in order:
/// 0. `assetRootOverridePath`, when a platform wrapper set it (Android - see above).
...
let repoRoot: URL = {
  if let overridden = assetRootOverridePath {
  ...
  return URL(fileURLWithPath: #filePath)
    .deletingLastPathComponent()  // GameMain.swift
...
/// The whole game: SDL init, window/renderer/audio setup, then the blocking
...
@GameActor public func junkbotMain() {
  ...
  // On Android, SDL's Java side finishes the Activity outright on the back button unless this
  // hint is set (see SDLActivity.onBackPressed) - trapping it instead lets the back button reach
  // the ordinary key-event path below (as SDLK_AC_BACK) so it can back out of a menu/level the
  // same way Escape does on desktop, rather than always quitting the app. No-op on every other
  ...
  // First access of the lazy `window`/`renderer`/`gameRenderer` globals - must come after
  // SDL.initialize and the `windowWidth`/`windowHeight` assignment above.
  //
  ...
  while running {
    while let event = SDL.pollEvent() {
      ...
      case .keyDown(_, let keycode):
        switch keycode {
        case Keycode(rawValue: SDLK_ESCAPE), Keycode(rawValue: SDLK_AC_BACK):
          // SDLK_AC_BACK is the Android back button (trapped via SDL_HINT_ANDROID_TRAP_BACK_BUTTON
          // above instead of being allowed to quit the app outright) - same semantics as Escape.
          escapePressed()
