...
/// drawing code live in exactly one shared file instead of two near-duplicates.
///
...
/// has real fields) - `OpaquePointer` is the one representation both backends can produce and
/// consume identically, and nothing here ever needs to read a texture's fields directly anyway.
///
...
protocol GameRenderer: AnyObject {
  ...
  /// Nearest-neighbor sampling - both SDL3 and SDL2 default new textures to linear filtering,
  /// which blurs pixel art whenever a sprite isn't drawn at exact 1:1 scale (i.e. almost always,
  /// given camera zoom/offset).
