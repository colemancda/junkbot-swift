...
struct Mesh {
  ...
  /// An octagonal stud (side ring + top cap, no bottom cap since it sits on a brick) - the cheaper
  /// per-brick decoration for the game renderer (`Models.brick`), 16 triangles vs `cylinderY`'s 24.
  mutating func stud(
    _ cx: Double, _ cz: Double, _ baseY: Double, _ radius: Double, _ height: Double,
    _ r: Int, _ g: Int, _ b: Int
  ) {
    ...
    for i in 0..<8 {
      ...
      emit(
        r, g, b,
        [
          vert((p0.0, baseY, p0.1), sideN), vert((p1.0, baseY, p1.1), sideN),
          vert((p1.0, topY, p1.1), sideN),
          vert((p0.0, baseY, p0.1), sideN), vert((p1.0, topY, p1.1), sideN),
          vert((p0.0, topY, p0.1), sideN),
        ])
      emit(
        r, g, b,
        [
          vert((cx, topY, cz), (0, 1, 0)), vert((p0.0, topY, p0.1), (0, 1, 0)),
          vert((p1.0, topY, p1.1), (0, 1, 0)),
        ])
    }
  }
  ...
  /// Returns a copy remapped for `Renderer3D`'s drop-in use: the model's X/Y bounding box is
  /// normalized to [-1,1] (so `Renderer3D.drawEntity`'s existing `glScalef32(halfW, halfH, ...)`
  /// stretches it to fill the entity's sprite footprint, exactly as it did the old unit cube),
  /// while Z is left in stud units (so the ~15px scale there keeps true depth - a thin panel stays
  /// thin). Y is negated because this file authors +Y up (nice for the standalone viewer) but the
  /// game world is +Y down; the normals' Y is negated to match, so the directional light still
  /// shades tops brightest. X/Z normals are unchanged (a positive per-axis normalize preserves
  /// axis-aligned normal directions, and round-part normals only shift slightly - acceptable).
  ///
  /// `bodyCenterYV16`/`bodyHalfYV16` override the Y normalization reference (default: the full
  /// bbox). Bricks pass their *body* extent here so the body alone fills the row and the studs
  /// protrude past +/-1 into the brick stacked above - otherwise the whole thing (studs included)
  /// would squash into one row, leaving a gap where the studs should connect.
  func unitModelForGame(bodyCenterYV16: Double? = nil, bodyHalfYV16: Double? = nil) -> Mesh {
    var minX = Int16.max, maxX = Int16.min, minY = Int16.max, maxY = Int16.min
    for part in parts {
      for v in part.tris {
        minX = Swift.min(minX, v.x); maxX = Swift.max(maxX, v.x)
        minY = Swift.min(minY, v.y); maxY = Swift.max(maxY, v.y)
      }
    }
    let cx = (Double(minX) + Double(maxX)) / 2
    let cy = bodyCenterYV16 ?? (Double(minY) + Double(maxY)) / 2
    let hx = Swift.max((Double(maxX) - Double(minX)) / 2, 1)
    let hy = bodyHalfYV16 ?? Swift.max((Double(maxY) - Double(minY)) / 2, 1)

    var out = Mesh()
    out.parts.reserveCapacity(parts.count)
    for part in parts {
      var t: [MeshVertex] = []
      t.reserveCapacity(part.tris.count)
      for v in part.tris {
        t.append(
          MeshVertex(
            x: Int16(((Double(v.x) - cx) / hx * 4096).rounded()),
            y: Int16((-(Double(v.y) - cy) / hy * 4096).rounded()),
            z: v.z,
            nx: v.nx, ny: -v.ny, nz: v.nz))
      }
      out.parts.append(MeshPart(r: part.r, g: part.g, b: part.b, tris: t))
    }
    return out
  }
