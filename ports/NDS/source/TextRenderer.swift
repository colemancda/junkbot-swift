...
/// Bitmap-font renderer for the DS top screen (info panel: level title/hint/moves, win/lose
...
/// case-mapping data tables, which this ROM's budget can't spare (see `Sources/JunkbotCore/
/// Level.swift`'s and this port's `Makefile`'s notes on the same issue), and every string this
/// port actually draws (level titles/hints, baked into `build/LevelAssets.swift` as
/// `StaticString` by `tools/LevelDump`) is plain ASCII anyway.

let fontBits: UnsafePointer<UInt8> =
  nds_asset_font_bin()!.assumingMemoryBound(to: UInt8.self)
let fontRowStride: Int32 = (fontBitmapWidth + 7) / 8

/// The DS's RGB555 grey used for the top screen's background fill.
let topScreenBackgroundColor: UInt16 = 0x8000 | (14 << 10) | (14 << 5) | 14
/// Near-black, for text over that grey.
let topScreenTextColor: UInt16 = 0x8000 | (2 << 10) | (2 << 5) | 2
...
func clearRect(x: Int32, y: Int32, width: Int32, height: Int32, into buffer: UnsafeMutablePointer<UInt16>) {
  var dy: Int32 = 0
  while dy < height {
    let py = y + dy
    if py >= 0, py < screenHeight {
      let rowBase = Int(py) * Int(screenWidth)
      var dx: Int32 = 0
      while dx < width {
        let px = x + dx
        if px >= 0, px < screenWidth {
          buffer[rowBase + Int(px)] = topScreenBackgroundColor
        }
        dx += 1
      }
    }
...
func drawChar(
  _ byte: UInt8, x: Int32, y: Int32, scale: Int32, color: UInt16,
...
@discardableResult
func drawText(
  _ text: StaticString, x: Int32, y: Int32, scale: Int32, color: UInt16,
  into buffer: UnsafeMutablePointer<UInt16>
) -> Int32 {
  ...
  while i < count {
    ...
    drawChar(byte, x: penX, y: y, scale: scale, color: color, into: buffer)
...
func drawWrappedText(
  ...
  while index < count {
    ...
    while i < wordEnd {
      let byte = byteAt(text, i)
      drawChar(byte, x: penX, y: penY, scale: scale, color: color, into: buffer)
      penX += glyphAdvance(byte) * scale
      i += 1
    }
    if wordEnd < count {
...
@discardableResult
func drawInt(
  _ value: Int32, x: Int32, y: Int32, scale: Int32, color: UInt16,
  into buffer: UnsafeMutablePointer<UInt16>
) -> Int32 {
  var digits: [UInt8] = []
  var v = max(value, 0)
