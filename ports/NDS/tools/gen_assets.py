#!/usr/bin/env python3
"""Build-time asset generator for the Nintendo DS port.

Reads the repo's canonical sprite tables (Sources/JunkbotCore/Generated/
SpriteTable.swift) and level listing (levels/_LEVEL_LISTING.txt), and emits:
  ...
  <out>/levels.bin         -- level .txt contents (BOM-stripped UTF-8),
                              concatenated in _LEVEL_LISTING.txt order.
  <out>/LevelAssets.swift  -- title/offset/length index into levels.bin.

Pure stdlib (zlib-based PNG decoding) so the build needs no pip installs.
Level-title matching replicates LevelCatalog.swift: listing order is the
source of truth, files are matched by their parsed `title=` line,
...
def average_rgb15(path):
    """Average opaque-pixel color as DS RGB555 (bit15 set), or 0 if none."""
...
def generate_sprites(repo_root, out_dir):
    ...
    (out_dir / "SpriteAssets.swift").write_text(
        ...
        "/// background-sheet images, which don't fit in DS RAM). Frame dimensions come\n"
        "/// from the core's `spriteWidthTable`/`spriteHeightTable`.\n"
        f"let spriteDataOffsetTable: [Int32] = [\n  {lines},\n]\n"
        "\n"
        "/// The shared sprite palette: index -> DS RGB555 (bit15 set). Index 0 is\n"
        ...
        "\n"
        "/// Average opaque-pixel color (DS RGB555, bit15 set) of each\n"
        ...
        "/// background images don't fit in DS RAM.\n"
...
def parse_title(level_text):
    for line in level_text.splitlines():
        if line.strip().lower().startswith("title="):
            return line.strip()[len("title=") :]
    return ""
...
def generate_levels(repo_root, out_dir):
    levels_dir = repo_root / "levels"
    by_title = {}
    for txt in levels_dir.glob("*.txt"):
        if txt.name == "_LEVEL_LISTING.txt":
            continue
        text = read_level_text(txt)
        key = normalized_title(parse_title(text)).lower()
        if key:
            by_title[key] = text

    blob = bytearray()
    entries = []
    listing = read_level_text(levels_dir / "_LEVEL_LISTING.txt")
    for line in listing.splitlines():
        title = normalized_title(line)
        if not title:
            continue
        text = by_title.get(title.lower())
        if text is None:
            print(f"warning: no level file for listing entry '{title}'", file=sys.stderr)
            continue
        encoded = text.encode("utf-8")
        entries.append((title, len(blob), len(encoded)))
        blob += encoded

    (out_dir / "levels.bin").write_bytes(blob)
    body = "\n".join(
        f"  EmbeddedLevel(title: {swift_string_literal(title)}, "
        f"offset: {offset}, length: {length}),"
        for title, offset, length in entries
    )
    (out_dir / "LevelAssets.swift").write_text(
        "// Generated by ports/NDS/tools/gen_assets.py - do not edit.\n"
        "\n"
        ...
        "}\n"
        "\n"
        f"let embeddedLevels: [EmbeddedLevel] = [\n{body}\n]\n"
    )
    print(f"levels.bin: {len(entries)} levels, {len(blob)} bytes")
...
def main():
    ...
    generate_levels(repo_root, out_dir)
