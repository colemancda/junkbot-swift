...
to signed 16-bit PCM mono at its native sample rate -- what NDSP's
`NDSP_FORMAT_MONO_PCM16` plays directly, no runtime decoder (see
source/Audio.swift / common/shim.c's ctru_play_pcm16). Unlike the DS port,
there's no ROM/RAM budget forcing an 8-bit/downsampled compromise here.
...
WAV (downmixing the one stereo source to mono using only the stdlib `wave`
module). WAV sources skip afconvert entirely. This step is therefore
macOS-only, matching this port's other toolchain assumptions.

Emits:
  <out>/audio.bin        -- every clip's PCM16 samples, concatenated in
                             SoundID order.
...
def decode_to_pcm16_mono(path, tmpdir):
    """Returns (sample_rate, bytes) as little-endian signed 16-bit PCM mono."""
    ...
    return rate, struct.pack(f"<{len(samples16)}h", *samples16)
...
def main():
    ...
    blob = bytearray()
    entries = []  # (offset, length, sampleRate) indexed by SoundID, bytes
    with tempfile.TemporaryDirectory() as tmp:
        ...
        for sound_id in range(max(SOUND_PATHS) + 1):
            ...
            entries.append((len(blob), len(pcm16), rate))
            blob += pcm16
    ...
    (out_dir / "AudioAssets.swift").write_text(
        ...
        "/// 16-bit PCM mono clip within `audio.bin`; `length == 0` for an unmapped ID.\n"
