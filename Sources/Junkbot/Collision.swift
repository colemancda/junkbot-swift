// Collision detection — faithful port of the JS collision code
...
// Returns a fake "levelBounds" entity if the rect hits the level boundary wall.
// Returns nil if out of bounds or not hitting a wall.
func rectangleLevelBoundsCollision(x: Int32, y: Int32, width: Int32, height: Int32) -> Entity? {
    ...
    e.fixed = true
    if x < bounds.x {
        e.x = bounds.x - CELL_W; e.y = bounds.y
        e.width = CELL_W; e.height = bounds.height
        return e
    }
    if y < bounds.y {
        e.x = bounds.x; e.y = bounds.y - CELL_H
        e.width = bounds.width; e.height = CELL_H
        return e
    }
    if x + width > bounds.x + bounds.width {
        e.x = bounds.x + bounds.width; e.y = bounds.y
        e.width = CELL_W; e.height = bounds.height
        return e
    }
    if y + height > bounds.y + bounds.height {
        e.x = bounds.x; e.y = bounds.y + bounds.height
        e.width = bounds.width; e.height = CELL_H
        return e
    }
    return nil
...
// Filter predicates — match the JS notDroplet / notBinOrDroplet patterns

func isNotDroplet(_ e: Entity) -> Bool { e.type != .droplet }

func isNotBinOrDroplet(_ e: Entity) -> Bool {
    e.type != .bin && e.type != .droplet
}

func isNotBinOrDropletOrEnemyBot(_ e: Entity) -> Bool {
    e.type != .bin && e.type != .droplet &&
    e.type != .gearbot && e.type != .climbbot &&
    e.type != .flybot && e.type != .eyebot
}

func isNotDropletOrJunkbot(_ e: Entity) -> Bool {
    e.type != .droplet && e.type != .junkbot
}

// First entity (excluding bounds-virtual and ignoring grabbed) whose rect intersects (x,y,w,h)
// and passes filter. Returns nil if none.
func rectangleCollisionTest(
    x: Int32, y: Int32, width: Int32, height: Int32,
    filter: (Entity) -> Bool,
    excluding: Int = -1
) -> Entity? {
    if let bounds = rectangleLevelBoundsCollision(x: x, y: y, width: width, height: height), filter(bounds) {
        return bounds
    }
...
// Same as rectangleCollisionTest but using entity's own width/height, excluding self
...
func entityMoved(index: Int) {
    ...
    // Remove from old buckets if present
    let topY = e.y
    let bottomY = e.y + e.height
    // Update top-Y bucket
    if var arr = entitiesByTopY[topY] {
        if !arr.contains(index) { arr.append(index); entitiesByTopY[topY] = arr }
    } else {
        entitiesByTopY[topY] = [index]
    }
    if var arr = entitiesByBottomY[bottomY] {
        if !arr.contains(index) { arr.append(index); entitiesByBottomY[bottomY] = arr }
    } else {
...
// connectsToFixed — checks if entity at given index is supported by a fixed entity via contact
func connectsToFixed(startIndex: Int, direction: Int32 = 0, ignoreIndices: [Int] = []) -> Bool {
    if let bounds = levelBounds {
        ...
        if e.y + e.height >= bounds.y + bounds.height {
            return true
        }
    }
    ...
    func search(fromIndex: Int) -> Bool {
        ...
        if let bounds = levelBounds, from.y + from.height >= bounds.y + bounds.height {
            return true
        }
        // Entities resting on top of `from` (their top = from's bottom)
        let above = entitiesByTopY[from.y + from.height] ?? []
        // Entities that `from` rests on (their bottom = from's top)
