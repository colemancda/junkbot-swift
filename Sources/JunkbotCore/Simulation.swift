extension GameEngine {
  ...
  func hurtJunkbot(index: Int, cause: Int32) {
    ...
    if junkbot.armored {
      if !junkbot.losingShield { junkbot.losingShield = true }
    } else {
  ...
  func simulateGravity() {
    for i in 0..<entities.count {
      let e = entities[i]
      guard
        !e.fixed && !e.grabbed && !e.floating && e.type != .droplet && e.type != .junkbot
      ...
      let notSettled =
        ...
        && !connectsToFixed(
          startIndex: i,
          direction: (e.type == .gearbot || e.type == .crate || e.type == .bin) ? 1 : 0)

      if notSettled {
        if entityCollisionTest(entityX: e.x, entityY: e.y, entityIndex: i, filter: isNotDroplet)
          != nil
        {
          continue
        }
        let cellDownY = e.y + CELL_H
        let hits = entityCollisionAll(
          entityX: e.x, entityY: cellDownY + 1, entityIndex: i, filter: isNotDroplet)
  ...
  func walk(junkbotIndex: Int) -> Bool {
    ...
    if let wall = stepOrWall {
      ...
      if posStepUpY - junkbot.y >= -CELL_H && posStepUpY - junkbot.y < 0
        && entityCollisionTest(
          entityX: posInFrontX, entityY: posStepUpY, entityIndex: junkbotIndex,
  ...
  func findLinkedTeleportIndex(for teleportIndex: Int) -> Int {
    ...
    for i in 0..<entities.count {
      ...
      if other.type == .teleport && other.teleportID == tp.teleportID { return i }
    }
    return -1
  }
  ...
  func simulateJunkbot(index: Int) {
    ...
    let aboveHead = entityCollisionTest(
      entityX: junkbot.x, entityY: junkbot.y - 1, entityIndex: index, filter: isNotDroplet)
    ...
    if let ah = aboveHead {
      headLoaded =
        ...
        || (!ah.fixed
          && !connectsToFixed(
            startIndex: entities.firstIndex(where: { $0.id == ah.id }) ?? 0,
            ignoreIndices: [index])
    ...
    if entityCollisionTest(
      entityX: junkbot.x, entityY: junkbot.y, entityIndex: index,
    ...
    if junkbot.floating {
      ...
      if entityCollisionTest(
        entityX: abovePos.x, entityY: abovePos.y, entityIndex: index,
        filter: isNotDroplet) == nil
      {
    ...
    if inAir || jumpStarting || unaligned {
      let dirX: Int32 =
        junkbot.momentumY < -2 ? 0 : (junkbot.momentumX > 0 ? 1 : (junkbot.momentumX < 0 ? -1 : 0))
      let dirY: Int32 = junkbot.momentumY > 0 ? 1 : (junkbot.momentumY < 0 ? -1 : 0)
      let newX = junkbot.x + dirX * CELL_W
      let newY = junkbot.y + dirY * CELL_H
      let collideXY =
        entityCollisionTest(
          entityX: newX, entityY: newY, entityIndex: index,
          filter: isNotDroplet) != nil
      if collideXY {
        let collideYOnly =
          entityCollisionTest(
            entityX: junkbot.x, entityY: newY,
            entityIndex: index, filter: isNotDroplet) == nil
        let collideXOnly =
          entityCollisionTest(
            entityX: newX, entityY: junkbot.y,
            entityIndex: index, filter: isNotDroplet) == nil
        if collideYOnly {
          junkbot.momentumX = 0
          junkbot.y = newY
    ...
    if junkbot.animationFrame % 5 == 4 {
      ...
      let cratesInFront = rectangleCollisionAll(
        ...
        filter: {
          $0.type == .crate && ($0.x + $0.width <= junkbot.x || junkbot.x + junkbot.width <= $0.x)
        })
      ...
      for i in 0..<entities.count {
        ...
        guard ground.x <= junkbot.x && ground.x + ground.width >= junkbot.x + junkbot.width else {
          continue
        }
        if !turnedAround {
          switch ground.type {
  ...
  func simulateClimbbot(index: Int) {
    ...
    if entities[index].animationFrame > 6 {
      ...
      let aside = entityCollisionTest(
        entityX: asideX, entityY: c.y, entityIndex: index, filter: isNotDroplet)
      let groundAside = entityCollisionTest(
        entityX: asideX, entityY: c.y + 1, entityIndex: index, filter: isNotBinOrDroplet)
      let ahead = entityCollisionTest(
        entityX: aheadPos.x, entityY: aheadPos.y, entityIndex: index, filter: isNotDroplet)
      let behindHoriz = entityCollisionTest(
        entityX: behindX, entityY: c.y, entityIndex: index, filter: isNotDroplet)
      let below = entityCollisionTest(
        entityX: belowPos.x, entityY: belowPos.y, entityIndex: index, filter: isNotDroplet)
  ...
  func simulateFlybot(index: Int) {
    ...
    if entities[index].animationFrame % 2 == 0 {
      ...
      let ahead = entityCollisionTest(
        entityX: aheadX, entityY: f.y, entityIndex: index,
  ...
  func doEyebotTargeting(index: Int) {
    ...
    for (dirX, dirY) in directions {
      ...
      if dirY != 0 {
        rayOffsets = [(0, 0), (CELL_W, 0)]
      } else {
        rayOffsets = [(0, 0), (0, CELL_H)]
      }
  ...
  func simulateScaredy(index: Int) {
    ...
    if entities[index].animationFrame > 2 {
      ...
      if let jb = junkbotHit {
        ...
        let ahead = entityCollisionTest(
          entityX: aheadX, entityY: bin.y, entityIndex: index,
          filter: isNotDroplet)
        if ahead != nil {
  ...
  func simulateDroplet(index: Int) {
    if entities[index].splashing {
      entities[index].animationFrame += 1
      if entities[index].animationFrame > 4 { entities[index].removeBeforeRender = true }
    } else {
      ...
      for _ in 0..<18 {
        ...
        for j in 0..<entities.count {
          if j == index { continue }
          let g = entities[j]
          if g.grabbed || g.type == .droplet { continue }
          if g.y != dy + droplet.height { continue }
          if dx + dw <= g.x || dx >= g.x + g.width { continue }
          if g.type == .junkbot { hurtJunkbot(index: j, cause: 2) }
  ...
  func simulateTeleport(index: Int) {
    ...
    if tp.timer > TELEPORT_COOLDOWN - TELEPORT_EFFECT_PERIOD {
      teleportEffects.append(TeleportEffect(x: tp.x + CELL_W, y: tp.y, frameIndex: tp.timer % 3))
    }
  }
          ...
          while y > -200 {
            ...
            for j in 0..<entities.count {
              ...
              if !rectanglesIntersect(
                x, y, CELL_W, CELL_H, other.x, other.y, other.width, other.height)
              {
                continue
              }
              if other.type == .junkbot {
                if !other.wasFloating { playSound(.fan) }
      ...
      if e.type == .laser && e.on {
        ...
        while running && extent < 200 {
          ...
          for j in 0..<entities.count {
            ...
            if other.grabbed { continue }
            if !rectanglesIntersect(
              lx, e.y, CELL_W, CELL_H, other.x, other.y, other.width, other.height)
            {
              continue
            }
  ...
  func simulate() {
    ...
    for i in 0..<entities.count {
      ...
      default:
        if entities[i].animationFrame != -1 { entities[i].animationFrame += 1 }
      }
    }
