extension GameEngine {
    ...
    func simulateGearbot(index: Int) {
        ...
        if entities[index].animationFrame > 2 {
            ...
            let ahead = entityCollisionTest(entityX: aheadX, entityY: gearbot.y, entityIndex: index,
                                            filter: isNotDroplet)
    ...
    func simulateClimbbot(index: Int) {
        ...
        if entities[index].animationFrame > 6 {
            ...
            let aside = entityCollisionTest(entityX: asideX, entityY: c.y, entityIndex: index, filter: isNotDroplet)
            let groundAside = entityCollisionTest(entityX: asideX, entityY: c.y + 1, entityIndex: index, filter: isNotBinOrDroplet)
            let ahead = entityCollisionTest(entityX: aheadPos.x, entityY: aheadPos.y, entityIndex: index, filter: isNotDroplet)
            let behindHoriz = entityCollisionTest(entityX: behindX, entityY: c.y, entityIndex: index, filter: isNotDroplet)
            let below = entityCollisionTest(entityX: belowPos.x, entityY: belowPos.y, entityIndex: index, filter: isNotDroplet)
    ...
    func simulateFlybot(index: Int) {
        ...
        if entities[index].animationFrame % 2 == 0 {
            ...
            let ahead = entityCollisionTest(entityX: aheadX, entityY: f.y, entityIndex: index,
    ...
    func doEyebotTargeting(index: Int) {
        ...
        for (dirX, dirY) in directions {
            ...
            if dirY != 0 { rayOffsets = [(0, 0), (CELL_W, 0)] } else { rayOffsets = [(0, 0), (0, CELL_H)] }
    ...
    func doEyebotMovement(index: Int) {
        ...
        if e.animationFrame % Int32(period) == 0 {
            ...
            let ahead = entityCollisionTest(entityX: aheadX, entityY: aheadY, entityIndex: index,
                                            filter: { $0.type != .droplet })
            if let a = ahead {
                if a.type == .junkbot, let jIdx = entities.firstIndex(where: { $0.id == a.id }) {
                    hurtJunkbot(index: jIdx, cause: 0)
    ...
    func simulateTeleport(index: Int) {
        ...
        if linkedIdx < 0 { entities[index].blocked = true; return }
            ...
            if e.type == .laser && e.on {
                ...
                while running && extent < 200 {
                    ...
                    for j in 0..<entities.count {
                        ...
                        if other.type != .droplet { hitIdx = j; running = false; break }
                    }
                    if running { extent += 1 }
                }
    ...
    func simulate() {
        ...
        if newState != winLoseState {
            ...
            if newState == 1 { playSound(.ohYeah) }
            else if newState == 2 { playSound(.ouch) }
        }
    }
}
