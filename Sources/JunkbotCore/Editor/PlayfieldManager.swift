...
public class PlayfieldManager: LingoObject, @unchecked Sendable {
  ...
  func toString() -> String {
    ...
    for part in partslist {
      ...
      let part_text =
        "\(pos.x);\(pos.y);\(type_num);\(color_num);\(state_name);\(frame_num);\(label_val)"
  ...
  func setPlayfield(_ pfinfo: LV, opt: LV = .void) {
    ...
    if let partsList = partsData.asList {
      for i in 1...max(1, partsList.count) {
        ...
        let part_type =
          part_typenum > 0 && part_typenum <= typeList.count ? typeList[part_typenum - 1] : ""
        let part_color: String? =
          part_colornum == 0
          ? nil : (part_colornum <= colorList.count ? colorList[part_colornum - 1] : nil)
        var part = PropList()
    ...
    for d in sh {
      ...
      if dir != "down" && pos.y > 1 {
        ...
        if n != 0, let nPart = partslist[n - 1] {
          ...
          if !propListIdentical(nPart, p) && !nei.contains(where: { propListIdentical($0, nPart) })
            && !exclude.contains(nType)
          {
            nei.append(nPart)
          }
        }
      ...
      if dir != "UP" && pos.y < pf_size[1] {
        ...
        if n != 0, let nPart = partslist[n - 1] {
          if !propListIdentical(nPart, p) && brickP(nPart)
            && !nei.contains(where: { propListIdentical($0, nPart) })
          {
            nei.append(nPart)
          }
        }
  ...
  // ```
  func partSupported(
    _ p: PropList, group: [PropList]? = nil, ignoregroup: [PropList] = [], recurse: Int = 1
  ) -> LV {
  ...
  func getPartsByType(_ typelist: LV) -> [PropList] {
    var tlist = [String]()
    if let single = typelist.asString {
      tlist = [single]
  ...
  func getPartsByLabel(_ labelList: LV) -> [PropList] {
    var llist = [String]()
    if let single = labelList.asString {
      llist = [single]
