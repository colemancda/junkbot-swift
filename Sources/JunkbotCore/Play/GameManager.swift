...
public class GameManager: LingoObject, @unchecked Sendable {
  ...
  public func prepareLevelMenu() {
    ...
    var level = 1
    let stubLevelText =
      "[info:[title:\"Stub Level\", hint:\"Walk to the exit!\", par:10], bricks:[[#BRICK_01, point(0,0)]]]"
    for n in 1...60 {
      let leveldata =
        (Glob.shared["config_manager"].asObject() as? BehaviorConfigManager)?.parseParams(
          stubLevelText) ?? PropList()
