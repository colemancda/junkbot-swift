...
class BehaviorMsgBoxTitle: LingoObject, @unchecked Sendable {
  ...
  func updateLoc(newloc: Point) {
    let sprites = prop["sprites"].asPropList!
    sprite(myNum).loc = newloc
    sprite(sprites["icon"].asInt!).loc =
      sprite(myNum).loc + Point(x: 48, y: 74) + Point(x: 30, y: -20)
  ...
  func fixLocZ() {
    ...
    sprite(sprites["icon"].asInt!).locZ = 1_000_000_001
    sprite(sprites["num"].asInt!).locZ = 1_000_000_001
    sprite(sprites["title"].asInt!).locZ = 1_000_000_001
  }
