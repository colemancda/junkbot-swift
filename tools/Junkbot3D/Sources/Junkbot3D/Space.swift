...
/// (+x right, +y **up**, +z toward camera). We use 1 game pixel = 1 scene unit, so a stud is
/// `CELL_W` (15) units wide and a brick row is `CELL_H` (18) units tall — the game's exact grid,
/// which also happens to preserve LEGO's real 5:6 stud-width:brick-height ratio.
enum Space {
